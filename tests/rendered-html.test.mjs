...
test("ships a structured course editor, reusable media library, and safe learner rendering", async () => {
  ...
  assert.match(builder, /CURRICULUM_SECTION_BATCH = 20/);
  assert.match(builder, /pinnedSectionIds = new Set/);
  assert.match(builder, /index < curriculumSectionLimit \|\| pinnedSectionIds\.has\(section\.id\)/);
  assert.match(builder, /Load more sections/);
  assert.match(builder, /The complete curriculum remains searchable/);
  assert.doesNotMatch(builder, /Math\.ceil\(\(sectionIndex \+ 1\) \/ CURRICULUM_SECTION_BATCH\)/);
