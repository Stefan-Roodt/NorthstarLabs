...
const lessons = [
  {
    id: "cmf-module-1-3-lesson-01",
    assetId: "cmf-module-1-3-double-spend-video",
    file: "module-1-3-digital-cash-problem.mp4",
    transcript: `Before Bitcoin, digital payment systems could already move value quickly. The difficult question was narrower: how could a native digital unit be transferred without a central operator preventing it from being spent twice?
...
This is not magic and it does not remove every intermediary. Wallets, exchanges, internet access and custody services can still introduce trust and risk. The protocol addresses agreement over ledger history. Use the conflict lab below to separate that achievement from claims it never made.`,
  },
  {
    id: "cmf-module-1-3-lesson-02",
    assetId: "cmf-module-1-3-ancestry-video",
    file: "module-1-3-technical-ancestry.mp4",
    transcript: `Bitcoin did not appear without ancestors. Its design combined decades of research into cryptography, distributed systems and digital money.
...
Do not reduce this ancestry to a single inventor or ingredient. The learning value lies in seeing how authorisation, integrity, ordering, incentives and verification reinforce one another. In the systems map below, match each component to its direct job, then explain why no component is sufficient on its own.`,
  ...
  {
    id: "cmf-module-1-3-lesson-03",
    assetId: "cmf-module-1-3-claims-video",
    file: "module-1-3-white-paper-claims.mp4",
    transcript: `The Bitcoin white paper is a design document, not an investment prospectus. Read its claims at the level they are made.
...
const guide = new URL("../public/media/course-resources/module-1-3-origins-of-bitcoin-evidence-lab.pdf", import.meta.url);
statements.push(`INSERT OR REPLACE INTO \`media_assets\` (\`id\`,\`school_id\`,\`owner_id\`,\`key\`,\`filename\`,\`content_type\`,\`size_bytes\`,\`kind\`,\`alt_text\`,\`created_at\`,\`updated_at\`) SELECT 'cmf-module-1-3-evidence-lab',c.\`school_id\`,c.\`owner_id\`,'static:/media/course-resources/module-1-3-origins-of-bitcoin-evidence-lab.pdf','Module 1.3 Origins of Bitcoin Evidence Lab.pdf','application/pdf',${statSync(guide).size},'document','Primary-source and system-design workbook for the origins of Bitcoin.',${createdAt},${createdAt} FROM \`courses\` c WHERE c.\`id\`=${sql(courseId)};`);
statements.push(`INSERT OR REPLACE INTO \`lesson_resources\` (\`id\`,\`lesson_id\`,\`asset_id\`,\`title\`,\`position\`) VALUES ('cmf-module-1-3-evidence-lab-link','cmf-module-1-3-lesson-03','cmf-module-1-3-evidence-lab','Origins of Bitcoin Evidence Lab',1);`);

await writeFile(new URL("../drizzle/0060_crypto_mastery_module_1_3_premium.sql", import.meta.url), `${statements.join("\n--> statement-breakpoint\n")}\n`, "utf8");
console.log("Wrote the Module 1.3 premium media and evidence-lab migration.");
