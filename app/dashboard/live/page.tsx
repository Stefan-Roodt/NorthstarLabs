...
function SessionCard({
  ...
  return <article className="panel live-session-card">
    ...
    <div className="live-session-main">
      <div className="live-session-title"><span className={`status ${session.status}`}>{session.status}</span><small>{session.productName || session.courseTitle}</small><h3>{session.title}</h3><p>{start.toLocaleString("en-ZA", { dateStyle: "full", timeStyle: "short" })} Â· {session.meetingProvider.replaceAll("_", " ")}</p></div>
      <div className="live-session-metrics"><span><b>{session.registrations || 0}</b> registered</span><span><b>{session.attended || 0}</b> attended</span><span><b>{session.capacity || "âˆž"}</b> capacity</span></div>
      <div className="live-session-actions">
        <a href={session.meetingUrl} target="_blank" rel="noreferrer">Open meeting â†—</a>
