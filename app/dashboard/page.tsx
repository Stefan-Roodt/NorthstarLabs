...
export default function Dashboard() {
  ...
  return <main className="system-shell"><aside className="system-side"><Link className="system-brand" href="/">✦ NORTHSTARLABS</Link><nav>{["Overview","Courses","Learners","Community","Products","Live","Plan","Analytics"].map(item=><button key={item} className={tab===item?"active":""} onClick={()=>setTab(item)}><span>{item==="Overview"?"⌂":item==="Courses"?"▣":item==="Learners"?"♙":item==="Community"?"◎":item==="Products"?"◇":item==="Live"?"◉":item==="Plan"?"R":"↗"}</span>{item}</button>)}<Link className="side-nav-link" href="/dashboard/academy"><span>✦</span>Storefront</Link><Link className="side-nav-link" href="/dashboard/integrations"><span>⇄</span>Integrations</Link><Link className="side-nav-link" href="/dashboard/operations"><span>⚙</span>Operations</Link></nav><div className="side-bottom"><Link href={profile?.activeSchool ? `/schools/${profile.activeSchool.slug}` : "/courses"}>{profile?.activeSchool ? "View my academy" : "View course marketplace"}</Link>{profile?.isPlatformAdmin&&<Link href="/admin">Platform administration</Link>}<a href="/account">Account settings</a><button onClick={signOut}>Sign out</button></div></aside><section className="system-main"><header className="system-top"><div><p className="sys-kicker">{profile?.activeSchool?.name || "YOUR ACADEMY"} · CREATOR WORKSPACE</p><h1>{tab}</h1></div><div className="user-chip"><span>{name.slice(0,2).toUpperCase()}</span><div><b>{name}</b><small>{profile?.activeSchool?.memberRole || "creator"} · {email}</small></div></div></header>
  {tab==="Overview"&&<><div className="metric-row"><article><span>Courses</span><strong>{courses.length}</strong><small>{published} published</small></article><article><span>Active learners</span><strong>{learners}</strong><small>Across all your courses</small></article><article><span>Product studio</span><strong>Live</strong><small>Bundles, memberships and cohorts</small></article></div><div className="system-grid"><article className="panel chart-panel empty-dashboard"><p className="sys-kicker">YOUR NEXT MOVE</p><h2>{courses.length ? "Package the complete outcome" : "Create your first course"}</h2><p>{courses.length ? "Combine courses, community and live sessions into a bundle or membership learners can join." : "Start with a title. You can add video, text, quizzes and pricing next."}</p><button className="sys-primary" onClick={()=>setTab(courses.length ? "Products" : "Courses")}>{courses.length ? "Build a product →" : "Create a course →"}</button></article><article className="panel activity-panel"><div className="panel-title"><b>Recent activity</b></div><div className="empty-activity"><strong>No learner activity yet</strong><p>Enrollments, product access, live attendance and community posts will appear here.</p></div></article></div></>}
