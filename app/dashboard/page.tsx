...
export default function Dashboard() {
  ...
  {tab==="Courses"&&<>
    ...
    <div className="action-bar"><div><h2>Your courses</h2><p>Build, publish, and improve every learning experience.</p><Link href="/dashboard/import">Already have material? Import courses, documents and learners free →</Link></div><form onSubmit={createCourse}><label htmlFor="new-course-title"><span>New private course</span><input id="new-course-title" required value={title} onChange={event=>setTitle(event.target.value)} placeholder="Name the result learners will achieve"/></label><button disabled={creating} className="sys-primary">{creating?"Creating…":"Create course draft"}</button></form></div>{notice&&<div className="notice">{notice}</div>}{courses.length===0?<div className="panel empty-courses"><h3>No courses yet</h3><p>Name the learner outcome above and NorthstarLabs will create a private draft, then take you straight to its editor.</p><Link className="sys-primary" href="/dashboard/import">Import existing material →</Link></div>:<div className="course-table"><div className="table-head"><span>Course</span><span>Students</span><span>Price</span><span>Status</span></div>{courses.map(course=><div className="table-row" key={course.id}><div><i>{course.title.slice(0,2).toUpperCase()}</i><a href={`/dashboard/courses/${course.id}`}><b>{course.title}</b><small className="edit-label">Open course editor →</small></a></div><span>{course.students}</span><span>{course.priceCents ? `$${(course.priceCents/100).toFixed(2)}` : "Free"}</span><span className={`status ${course.status}`}>{course.status}</span></div>)}</div>}
  </>}
