...
export default function Dashboard() {
  ...
  if (loading) return <main className="system-loading"><div><b>✦ NORTHSTAR</b><p>Preparing your creator workspace…</p></div></main>;

  const email = profile?.email ?? "";
  const name = profile?.displayName ?? email.split("@")[0];
  return <main className="system-shell"><aside className="system-side"><a className="system-brand" href="/">✦ NORTHSTAR</a><nav>{["Overview","Courses","Community","Memberships","Analytics"].map(item=><button key={item} className={tab===item?"active":""} onClick={()=>setTab(item)}><span>{item==="Overview"?"⌂":item==="Courses"?"▣":item==="Community"?"◎":item==="Memberships"?"◇":"↗"}</span>{item}</button>)}</nav><div className="side-bottom"><a href="/learn/brand-strategy">View learner portal</a><button onClick={signOut}>Sign out</button></div></aside><section className="system-main"><header className="system-top"><div><p className="sys-kicker">CREATOR WORKSPACE</p><h1>{tab}</h1></div><div className="user-chip"><span>{name.slice(0,2).toUpperCase()}</span><div><b>{name}</b><small>{email}</small></div></div></header>
  ...
  {tab==="Community"&&<ComingSoon title="Community" text="Create spaces, publish posts and bring your learners together."/>}{tab==="Memberships"&&<SubscriptionPanel/>}{tab==="Analytics"&&<ComingSoon title="Analytics" text="Real enrollment, engagement and revenue reporting will appear here."/>}</section></main>;
}
