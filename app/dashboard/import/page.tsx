  ...
  return <main className="import-page">
    ...
    <section className="import-workspace">
      ...
      {preview && plan && summary && <section className="import-preview">
        ...
        {plan.learners.length > 0 && <label className="import-invite-choice"><input type="checkbox" checked={sendInvitations} onChange={(event) => setSendInvitations(event.target.checked)} /><span><b>Create and queue {plan.learners.length} secure learner invitation{plan.learners.length === 1 ? "" : "s"}</b><small>Imported courses stay private drafts, so these invitations grant academy access only. Assign a course after it passes review and is published. Leave this off to import without contacting learners.</small></span></label>}
