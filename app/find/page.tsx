    ...
    <main className="navigator-page">
      ...
      <section className="navigator-course-picker" aria-labelledby="academy-directory-title">
        <div className="navigator-course-picker-heading">
          <div>
            <p className="sys-kicker">ACADEMIES</p>
            <h2 id="academy-directory-title">Your academy. Your modules.</h2>
            <p>Open an academy to see only the modules it offers. Every module shows its syllabus before you join.</p>
          </div>
          <Link href="/courses">See every module {"\u2192"}</Link>
        </div>
        <div className="navigator-course-grid">
          {academies.map((academy) => (
            <article key={academy.slug}>
      ...
      <section className="navigator-direct" aria-labelledby="coach-title">
        <div>
          <p className="sys-kicker">NEED INDIVIDUAL HELP?</p>
          <h2 id="coach-title">A coach is the tailored option.</h2>
          <p>Modules follow a defined syllabus. Coaching is where a person can adapt the pace, examples, and support to your real need.</p>
        </div>
        <div className="navigator-direct-actions"><Link className="button" href="/tutors">Meet a coach {"\u2192"}</Link></div>
      </section>

      <footer className="navigator-footer">
        <Link className="system-brand" href="/">* NORTHSTARLABS</Link>
        <p>Academies. Modules. Progress.</p>
        <div><Link href="/legal/terms">Terms</Link><Link href="/legal/privacy">Privacy</Link></div>
      </footer>
    </main>
  );
}
