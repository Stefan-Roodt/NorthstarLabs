...
export function DemandBoard({ initialTopics }: { initialTopics: PublicDemandTopic[] }) {
  ...
  return <main className="demand-page">
    ...
    {managedFollow && <section className="demand-manage"><div><b>Email updates for “{managedFollow.title}”</b><span>{managedFollow.status === "active" ? "You will hear when its roadmap status changes." : "Updates are currently stopped."}</span></div>{managedFollow.status === "active" && <button disabled={busy === "unfollow"} onClick={() => void unfollow()}>{busy === "unfollow" ? "Stopping…" : "Stop email updates"}</button>}</section>}
    ...
    <section className="demand-board" id="board">
      ...
      <div className="demand-list">
        ...
        {topics.map((topic, index) => <article id={`demand-${topic.id}`} className={`demand-card status-${topic.status}`} key={topic.id}>
          <div className="demand-rank"><span>{String(index + 1).padStart(2, "0")}</span><div><button aria-label={`Support ${topic.title}`} aria-pressed={topic.viewerVote === 1} disabled={busy === `vote-${topic.id}`} onClick={() => void vote(topic, 1)}>↑</button><strong>{topic.score}</strong><button aria-label={`Do not support ${topic.title}`} aria-pressed={topic.viewerVote === -1} disabled={busy === `vote-${topic.id}`} onClick={() => void vote(topic, -1)}>↓</button></div></div>
